...
def check_and_do():
    if try_login(login_url):
        ...
        print 'login fail...'
def check_new_day():
    newday = datetime.datetime.replace(datetime.datetime.now() + datetime.timedelta(days = 1),hour = 0,minute = 0,second = 0)
    delta = newday - datetime.datetime.now()
    #time.sleep(3)#test
    #time.sleep(delta.seconds)
    ######not sleep but roter instead###########
    rotor  = "|/-\\|/-\\"
    them = rotor
...
if __name__ =="__main__":
    ...
    while True:
        print 'Sleeping...'
        if check_new_day():
            username = sys.argv[-2] ###账号###
            password = sys.argv[-1] ###密码###
            check_and_do()
            logindays += 1
            print 'Keep on logining on for %s days!' % (logindays)
